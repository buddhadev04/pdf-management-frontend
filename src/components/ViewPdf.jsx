...
import { fetchCategories, fetchFiles, verifyPdfAccess, checkApproval } from '../services/apiService';
...
const ViewPdf = () => {
  ...
  useEffect(() => {
    if (selectedCategory) {
      const getFiles = async () => {
        try {
          const data = await fetchFiles(selectedCategory);
          setFiles(data);
        } catch (error) {
  ...
  const handlePasswordSubmit = async () => {
    try {
      const data = await verifyPdfAccess(selectedFileId, password);
      ...
      const interval = setInterval(async () => {
        const checkData = await checkApproval(data.token);
        ...
        {pdfId === 'all' ? (
          <>
            ...
            {!selectedCategory ? (
              ...
              <>
                <button className="back-button casco" onClick={() => setSelectedCategory(null)}>Back to Folders</button>
